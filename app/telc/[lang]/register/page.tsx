...
export default function RegisterPage({ params }: PageProps) {
  ...
  // Prefetch exam data immediately on page load so step 2 is instant
  const [preloadedExams, setPreloadedExams] = useState<any[] | undefined>(undefined)
  const [preloadedLevels, setPreloadedLevels] = useState<any[] | undefined>(undefined)
  useEffect(() => {
    Promise.all([
      fetch("/api/telc/exams?future=true").then(r => r.json()).catch(() => []),
      fetch("/api/telc/exam-levels").then(r => r.json()).catch(() => []),
    ]).then(([exams, levels]) => {
      setPreloadedExams(Array.isArray(exams) ? exams : [])
      setPreloadedLevels(Array.isArray(levels) ? levels : [])
    })
  }, [])
      ...
      <div className="container mx-auto max-w-2xl px-4 py-10">
        ...
        {step === 3 && registrationId && (
          <StepPayment
            ...
            lang={activeLang}
            onPaymentComplete={() => {
              handleDataChange({ registeredAt: new Date().toISOString() })
              setStep(4)
            }}
