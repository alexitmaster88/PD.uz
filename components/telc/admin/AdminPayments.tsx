...
export default function AdminPayments({ lang }: Props) {
  ...
  const handleAction = async (payment: any, action: "accept" | "cancel") => {
    ...
    try {
      ...
      toast.success(action === "accept" ? t("payment_accepted") : t("payment_cancelled"))
      load()
      if (selected?.id === payment.id) setSelected(null)
    } catch {
      toast.error(t("payment_action_failed"))
    } finally {
      setActingId(null)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-slate-500">{payments.length} {t("total")}</span>
      </div>

      {selected && (
        <div className="rounded-xl border border-slate-200 bg-white p-6">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-base font-semibold text-slate-900">{t("payment_label")} #{selected.id}</h3>
          ...
          <div className="grid sm:grid-cols-2 gap-4 text-sm mb-5">
            {[
              [t("col_registrant"), `${selected.registrations?.first_name ?? ""} ${selected.registrations?.last_name ?? ""}`],
              [t("col_email"), selected.registrations?.email ?? "—"],
              [t("col_phone"), selected.registrations?.phone_number ?? "—"],
              [t("col_passport"), selected.registrations?.passport_number ?? "—"],
              [t("col_amount"), fmt(selected.amount)],
              [t("col_method"), selected.payment_method],
              [t("col_gateway"), selected.gateway ?? "manual"],
              [t("col_transaction"), selected.transaction_id ?? "—"],
              [t("col_payer_inn"), selected.payer_inn ?? "—"],
              [t("col_check_id"), selected.check_id ?? "—"],
              [t("col_created"), fmtDate(selected.created_at)],
              [t("col_verified"), selected.verified_at ? fmtDate(selected.verified_at) : "—"],
            ].map(([label, value]) => (
              <div key={label}>
      ...
      <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
        {loading ? (
          ...
          <p className="p-8 text-center text-slate-500">{t("no_payments")}</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  {[t("col_registrant"), t("col_amount"), t("col_method"), t("col_status"), t("col_date"), t("col_actions")].map(h => (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {p.status === "pending" && (
                          <>
                            ...
                            >
                              {actingId === p.id ? <Loader2 size={12} className="animate-spin" /> : <CheckCircle size={12} />} {t("btn_accept")}
                            </button>
                            <button
                            ...
                            >
                              {actingId === p.id ? <Loader2 size={12} className="animate-spin" /> : <XCircle size={12} />} {t("btn_cancel_payment")}
                            </button>
                          </>
                        )}
                      </div>
