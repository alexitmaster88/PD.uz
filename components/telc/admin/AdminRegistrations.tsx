...
import { Eye, X, Loader2, Pencil, Trash2 } from "lucide-react"
...
export default function AdminRegistrations({ lang }: Props) {
  ...
  const STATUSES = ["pending","verified","paid","completed","cancelled"]

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-slate-900">{t("registrations")}</h2>
        <span className="text-sm text-slate-500">{registrations.length} {t("total")}</span>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  {[t("col_name"), t("col_email"), t("col_phone"), t("col_passport"), t("label_level"), t("col_status"), t("col_registered_at"), t("col_actions")].map(h => (
                ...
                {registrations.map((reg: any) => (
                  <tr key={reg.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-slate-500 text-xs whitespace-nowrap">
                      {reg.created_at ? new Date(reg.created_at).toLocaleString("ru-RU", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "—"}
                    </td>
                    <td className="px-4 py-3">
