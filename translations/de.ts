const de = {
  // Hero Section
  welcome: "Willkommen im „PROFI DEUTSCH“ Bildungs-und Prüfungszentrum in Usbekistan",
  hero_tagline: "Profi Deutsch – die richtige Wahl, das exakte Ergebnis.",
  discover:
    "Lernen Sie Deutsch strukturiert und effizient nach der modernen telc Methode und sichern Sie sich die Chance auf ein anerkanntes internationales telc Zertifikat. Darüber hinaus bieten wir Ihnen gemeinsam mit unseren deutschen Partnern praxisnahe Unterstützung rund um Ausbildung und Beruf.",
  explore_courses: "Kursangebot",
  contact_us: "Kontakt",

  // Header
  courses: "Kurse",
  locations: "Standorte",
  partners: "Partner",
  telc: "telc",
// benefits: "Vorteile",
  contact: "Kontakt",
  register_now: "Jetzt Anmelden",
  left_menu_title: "Schnellmenü",
  left_menu_home: "Startseite",
  left_menu_telc: "telc Anmeldung",
  left_menu_telc_booking: "telc Buchung",
  left_menu_contact: "Kontakt",

  // telc Page
  telc_page_title: "Offizielle telc Prüfungsplattform in Usbekistan",
  telc_page_subtitle: "Internationale Zertifizierung für Deutschlernende",
  telc_page_description: "Buchen Sie Ihre telc Prüfung online mit der passenden Sprachversion und erhalten Sie Unterstützung von unserem zertifizierten Team.",
  telc_feature_1: "Offizielle telc Prüfungsanmeldung",
  telc_feature_2: "Vorbereitungsunterstützung und Materialien",
  telc_feature_3: "Personalisierte Unterstützung für Ihren Erfolg",
  telc_feature_easy_title: "Einfache Anmeldung",
  telc_feature_easy_desc: "Melden Sie sich in wenigen Minuten für Ihre Prüfung an",
  telc_feature_secure_title: "Sichere Zahlung",
  telc_feature_secure_desc: "Mehrere Zahlungsmethoden für Ihren Komfort",
  telc_feature_cert_title: "Offizielles Zertifikat",
  telc_feature_cert_desc: "Erhalten Sie Ihr internationales telc Zertifikat",
  telc_register_for_exam: "Zur Prüfung anmelden",
  telc_cta: "Zur telc Anmeldung",
  telc_back_to_home: "Zurück zur Hauptseite",
  telc_back_to_registration: "Zurück zur telc Anmeldeseite",
  telc_page_info_label: "telc Anmeldung",
  telc_page_info_text: "Erhalten Sie einen offiziellen telc Prüfungstermin mit voller Unterstützung durch unser Team in Ihrer Sprache.",
  telc_page_ticket_label: "Prüfungsniveaus",
  telc_page_ticket_title: "telc Deutsch Niveaus",
  telc_page_ticket_subtitle: "Wählen Sie Ihren telc Deutsch-Weg von A1-A2 bis B2.",
  telc_page_stat_1_title: "Sprachniveaus",
  telc_page_stat_1_value: "A1–B2",
  telc_page_stat_2_title: "Jetzt starten",
  telc_page_stat_2_value: "Schnelle Anmeldung",
  telc_page_level_a1_a2: "A1-A2",
  telc_page_level_a1_a2_description: "Vom Anfänger- bis Elementarbereich für die Alltagskommunikation.",
  telc_page_level_a2_b1: "A2-B1",
  telc_page_level_a2_b1_description: "Vom Elementar- bis Mittelstufenniveau für Alltag und Arbeit.",
  telc_page_level_b1: "B1",
  telc_page_level_b1_description: "Selbstständiges Deutsch für Studium und Beruf.",
  telc_page_level_b2: "B2",
  telc_page_level_b2_description: "Fortgeschrittenes Deutsch für berufliche und akademische Zwecke.",
  telc_step_1_title: "Wählen Sie Ihr Niveau",
  telc_step_1_description: "Wählen Sie das telc Deutsch Prüfungsniveau, das am besten zu Ihren Zielen und Ihrem aktuellen Kenntnisstand passt.",
  telc_step_2_title: "Reichen Sie Ihre Dokumente ein",
  telc_step_2_description: "Senden Sie uns Ihre Anmeldedaten und Ihren Wunschprüfungstermin, damit wir Ihren Platz buchen können.",
  telc_step_3_title: "Erhalten Sie die Bestätigung",
  telc_step_3_description: "Wir bestätigen Ihren Prüfungstermin und stellen Ihnen die notwendigen Informationen und Materialien zur Verfügung.",
  telc_faq_section_label: "FAQ",
  telc_faq_title: "Fragen zur telc Anmeldung",
  telc_faq_description: "Antworten auf die häufigsten Fragen zur telc Deutsch Prüfungsanmeldung.",
  telc_faq_q_1: "Was ist die telc Deutschprüfung?",
  telc_faq_a_1: "telc ist eine international anerkannte Deutschprüfung mit Niveaus von A1-A2 bis B2, geeignet für Alltag, Ausbildung und Arbeit.",
  telc_faq_q_2: "Für welche Niveaus kann ich mich anmelden?",
  telc_faq_a_2: "Sie können sich für telc Deutsch Prüfungen von A1-A2 bis B2 anmelden. Wir helfen Ihnen, das richtige Niveau basierend auf Ihrem aktuellen Sprachniveau zu wählen.",
  telc_faq_q_3: "Wie buche ich meine Prüfung?",
  telc_faq_a_3: "Füllen Sie Ihre Anmeldeanfrage aus, wählen Sie Ihren gewünschten Prüfungstermin und wir übernehmen die offizielle Buchung mit telc Partnern.",
  telc_faq_q_4: "Wann erhalte ich das Prüfungsergebnis?",
  telc_faq_a_4: "telc Prüfungsergebnisse sind in der Regel einige Wochen nach der Prüfung verfügbar. Wir informieren Sie sofort, sobald die Ergebnisse vorliegen.",
  telc_need_help_label: "Brauchen Sie Hilfe?",
  telc_need_help_title: "Haben Sie noch Fragen zur telc?",
  telc_need_help_cta: "Kontaktieren Sie unser Team",
  telc_booking_title: "telc Deutsch Prüfungsbuchung",
  telc_booking_subtitle: "Buchen Sie Ihre telc Deutschprüfung von A1 bis B2 in Usbekistan mit PROFI DEUTSCH.",
  telc_booking_description: "Wählen Sie Ihren bevorzugten Standort und Ihr Prüfungsniveau aus und senden Sie eine Buchungsanfrage an unser Team.",
  telc_booking_location_label: "Wählen Sie Ihren bevorzugten Standort",
  telc_booking_level_label: "Wählen Sie Ihr Prüfungsniveau",
  telc_booking_submit: "Buchungsanfrage senden",
  telc_booking_note: "Unser Team meldet sich bei Ihnen mit verfügbaren Prüfungsterminen und Anmeldedetails.",
  telc_booking_level_a1: "A1",
  telc_booking_exam_section_title: "telc Prüfungsoptionen",
  telc_booking_exam_section_subtitle: "Wählen Sie das Zertifikatsniveau, das Ihrem aktuellen Deutschkenntnisstand entspricht.",
  telc_booking_exam_levels_range: "A1-A2 · A2-B1 · B1 · B2",
  telc_booking_exam_a1: "telc Deutsch A1",
  telc_booking_exam_a2: "telc Deutsch A2",
  telc_booking_exam_b1: "telc Deutsch B1",
  telc_booking_exam_b2: "telc Deutsch B2",
  telc_booking_exam_desc_a1: "Grundlegende Prüfung für Anfänger mit Alltagssituationen.",
  telc_booking_exam_desc_a2: "Elementare Prüfung für praktische Gespräche und einfache Kommunikation.",
  telc_booking_exam_desc_b1: "Zwischenprüfung für selbstständige Nutzung von Deutsch im Beruf und Studium.",
  telc_booking_exam_desc_b2: "Oberstufenprüfung für professionelle Kommunikation und fortgeschrittenen Sprachgebrauch.",
  telc_booking_feature_official: "Offizielle telc Prüfungsanmeldung",
  telc_booking_feature_support: "Vorbereitungsunterstützung und Materialien",
  telc_booking_feature_personalized: "Personalisierte Vorbereitung für Ihren Erfolg",
  telc_booking_feature_fast_confirmation: "Schnelle Terminbestätigung",
  telc_booking_cities_title: "Verfügbare telc Prüfungsorte",
  telc_booking_cities_description: "Wählen Sie einen unserer Standorte in Usbekistan mit lokalem Support und Anmeldehilfe.",
  telc_booking_city_description: "Lokale telc Prüfungsunterstützung mit fachkundiger Beratung und Registrierung.",
  telc_booking_city_support_badge: "Prüfungsunterstützung",
  telc_booking_city_badge: "telc Standort",
  telc_booking_city_tashkent: "Taschkent",
  telc_booking_city_samarkand: "Samarkand",
  telc_booking_city_fergana: "Fergana",
  telc_booking_city_bukhara: "Buchara",
  telc_booking_city_khorezm: "Choresm",
  telc_booking_city_qashqadaryo: "Kashkadarya",
  telc_booking_city_qarshi: "Qarshi",
  telc_booking_city_nukus: "Nukus",
  telc_booking_level_a2: "A2",
  telc_booking_level_b1: "B1",
  telc_booking_level_b2: "B2",
  telc_booking_tab_information: "Information",
  telc_booking_tab_preparation: "Vorbereitungskurs",
  telc_booking_tab_faq: "FAQ",
  telc_booking_info_b1_b2_title: "Was können Sie auf B1- und B2-Niveau?",
  telc_booking_info_b1_b2_text: "Auf dem B1-Niveau können Sie sich einfach und zusammenhängend zu vertrauten Themen und persönlichen Interessensgebieten äußern. Auf dem B2-Niveau können Sie die Hauptinhalte komplexer Texte zu konkreten und abstrakten Themen verstehen. Sie können auch spontan und fließend kommunizieren.",
  telc_booking_info_exam_structure_title: "Wie ist die Prüfung aufgebaut?",
  telc_booking_info_exam_structure_intro: "Die telc Deutsch B1-B2 Prüfung besteht aus einem schriftlichen und einem mündlichen Teil. Die schriftliche Prüfung dauert 1 Stunde und 55 Minuten. Vor der mündlichen Prüfung haben Sie 20 Minuten Vorbereitungszeit.",
  telc_booking_info_written_title: "Schriftliche Prüfung",
  telc_booking_info_written_overview: "Die schriftliche Prüfung umfasst Hören, Lesen / Sprachbausteine und schriftliche Aufgaben.",
  telc_booking_info_written_listening: "Hören – 25 Min",
  telc_booking_info_written_reading: "Lesen / Sprachbausteine – 60 Min",
  telc_booking_info_written_writing: "Schriftlicher Ausdruck – 45 Min",
  telc_booking_info_oral_title: "Mündliche Prüfung",
  telc_booking_info_oral_overview: "Die mündliche Prüfung folgt einer kurzen Vorbereitungszeit und findet meist mit einem anderen Kandidaten statt.",
  telc_booking_info_oral_preparation: "Vorbereitungszeit – 20 Min",
  telc_booking_info_oral_exam: "Mündliche Prüfung meist mit einem weiteren Kandidaten",
  telc_booking_preparatory_course_title: "Vorbereitungskurs",
  telc_booking_preparatory_course_text: "Unser telc Vorbereitungskurs umfasst gezieltes Training für schriftliche und mündliche Prüfungsabschnitte sowie Prüfungsstrategien und Selbstvertrauen.",
  telc_booking_preparatory_course_question: "Suchen Sie einen telc Vorbereitungskurs oder einen telc Crashkurs für die Prüfung telc Deutsch B1-B2 Pflege? Das Profi-deutsch Zentrum in Usbekistan bietet Ihnen die ideale Vorlage für eine erfolgreiche Prüfung.",
  telc_booking_preparation_bullet_1: "Profi-deutsch Usbekistan bereitet Sie speziell auf die vier Prüfungsabschnitte vor (Leseverstehen, Hörverstehen, Schriftlicher Ausdruck und Mündlicher Ausdruck)",
  telc_booking_preparation_bullet_2: "Für alle Kurse bieten wir Online- und Präsenzformate an",
  telc_booking_preparation_bullet_3: "Unsere qualifizierten Trainer geben Ihnen einen Überblick über Aufbau und Format der telc Prüfung",
  telc_booking_preparation_bullet_4: "Erprobte Tipps und Strategien werden anhand von Modelltests erklärt",
  telc_booking_preparation_bullet_5: "Der Kurs findet in kleinen Gruppen statt, was individuelle Betreuung ermöglicht",
  telc_booking_preparation_bullet_6: "Alle Bereiche der telc Prüfung werden trainiert und geübt",
  telc_booking_preparation_bullet_7: "Sie arbeiten mit telc Übungstests (telc Modelltests)",
  telc_booking_preparatory_course_contact_prompt: "Möchten Sie sich auf die telc Deutsch B1-B2 Pflege Prüfung vorbereiten? Dann nehmen Sie Kontakt mit uns auf:",
  telc_booking_faq_intro: "Ihre Fragen – Unsere Antworten",
  telc_booking_faq_link_text: "Haben Sie Fragen zur telc Prüfung \u201etelc Deutsch B1-B2 Pflege\u201c? Mit dem folgenden",
  telc_booking_faq_link_label: "Link",
  telc_booking_faq_link_followup: "kommen Sie zu unserer telc FAQ Seite. Hier finden Sie Antworten auf die am häufigsten gestellten Fragen.",
  telc_booking_faq_support_text: "Sollten Sie weitere Fragen haben, dann schreiben Sie uns. Wir helfen Ihnen gerne!",

  // Course Types
  course_type_german: "Niveaustufe",
  course_type_uzbek: "telc Prüfung",
  course_type_english: "Ausbildung-Training",
  course_type_russian: "Beratungsservice",

  // Language Selection
  language_german_course: "Deutsch A1-A2",
  language_uzbek_course: "Deutsch B1-B2",
  language_english_course: "telc Zertifikat",
  language_russian_course: "Ausbildung-Training",

  // Courses Section
  our_courses: "Unsere Deutschkurse",
  // choose_courses:
  //   "Wählen Sie aus unseren spezialisierten Deutschkursen und Zertifizierungsprogrammen.",
  german_a1_a2: "Deutsch A1-A2",
  german_b1_b2: "Deutsch B1-B2",
  telc_certificate: "telc Zertifikat",
  ausbildung_training: "Ausbildung-Training",
  hours: "Unterrichtsstunden",
  small_groups: "Kleine Gruppen",
  // certificate: "Zertifikat nach erfolgreichem Abschluss",
  materials: "Lehrmaterial inklusive",

  // Map Section
  our_locations: "Unsere Standorte",
  discover_centers:
    "Entdecken Sie unsere Sprachzentren in ganz Usbekistan. Klicken Sie auf die Karte, um mehr zu erfahren.",
  tashkent_city: "Deutsch A1-A2",
  tashkent_region: "Deutsch B1-B2",
  telc_center: "telc Zertifikat",
  ausbildung_center: "Ausbildung-Training",
  address: "Adresse:",
  phone: "Telefon:",
  email: "E-Mail:",
  available_courses: "Verfügbare Kurse:",

  // Benefits Section
  // why_learn_german: "Warum ist es wichtig, Deutsch zu lernen?",
  // discover_benefits:
  //   "Entdecken Sie die Vorteile des Deutschlernens und wie unsere Kurse Ihnen dabei helfen können, Ihre Ziele zu erreichen.",
  // international_recognition: "Internationale Anerkennung",
  // modern_methods: "Moderne Lehrmethoden",
  // qualified_teachers: "Qualifizierte Lehrkräfte",
  // small_learning_groups: "Kleine Lerngruppen",
  // career_prospects: "Berufliche Perspektiven",
  // study_opportunities: "Studienmöglichkeiten",

  // Benefits Section - Descriptions
  international_recognition_desc:
    "Unsere Zertifikate werden international anerkannt und können für Studium und Beruf verwendet werden.",
  modern_methods_desc: "Wir verwenden die neuesten Lehrmethoden und -materialien für einen effektiven Spracherwerb.",
  qualified_teachers_desc:
    "Unsere Lehrkräfte sind hochqualifiziert und haben langjährige Erfahrung im Sprachunterricht.",
  small_learning_groups_desc:
    "Kleine Gruppen ermöglichen eine individuelle Betreuung und einen schnelleren Lernfortschritt.",
  career_prospects_desc: "Deutschkenntnisse eröffnen Ihnen neue berufliche Perspektiven in Deutschland und weltweit.",
  study_opportunities_desc: "Mit unseren Sprachkursen bereiten Sie sich optimal auf ein Studium in Deutschland vor.",

  // // Benefits Section - Stats
  // german_most_spoken: "Deutsch ist die meistgesprochene Muttersprache in Europa",
  // german_importance:
  //   "Mit über 100 Millionen Muttersprachlern ist Deutsch eine der wichtigsten Sprachen in Europa und weltweit. Deutschland ist zudem die größte Volkswirtschaft in Europa und bietet zahlreiche Möglichkeiten für Studium und Karriere.",
  // million: "Mio.",
  // native_speakers: "Muttersprachler",
  // economy_europe: "Wirtschaft in Europa",
  // universities: "Universitäten",
  // german_companies: "Deutsche Unternehmen weltweit",

  // Contact Section
  contact_us_title: "Kontaktieren Sie uns",
  contact_questions:
    "Haben Sie Fragen zu unseren Kursen oder möchten Sie sich anmelden? Kontaktieren Sie uns noch heute.",
  send_message: "Senden Sie uns eine Nachricht",
  name: "Name",
  phone_number: "Telefon",
  subject: "Betreff",
  message: "Nachricht",
  send: "Nachricht senden",
  contact_info: "Kontaktinformationen",
  opening_hours: "Öffnungszeiten",
  monday_friday: "Montag - Freitag: 9:00 - 18:00 Uhr",
  saturday: "Samstag: 9:00 - 14:00 Uhr",
  sunday: "Sonntag: Geschlossen",
  headquarters: "Hauptsitz",

  // Footer
  about_us:
    "Ihr Partner für Sprachunterricht in Usbekistan. Wir bieten Kurse in Deutsch, Usbekisch, Englisch und Russisch an.",
  links: "Links",
  all_rights_reserved: "Alle Rechte vorbehalten.",

  // YouTube Player
  video: "Video",
  play: "Abspielen",
  pause: "Pausieren",
  mute: "Stummschalten",
  unmute: "Ton einschalten",

  // Photo Gallery
  photo_gallery_title: "Eindrücke aus unserem Sprachzentrum",
  previous: "Vorheriges Bild",
  next: "Nächstes Bild",

  // Home Page
  welcome_center: "Willkommen in unserem Sprachzentrum",
  welcome_center_tagline: "Profi Deutsch – die richtige Wahl, das exakte Ergebnis.",
  welcome_center_description: "Lernen Sie Deutsch systematisch mit der modernen telc Methode und nutzen Sie die Gelegenheit, ein internationales telc Zertifikat zu erhalten. Erhalten Sie darüber hinaus praktische Unterstützung in Bezug auf Ihre Studien und Arbeit in Zusammenarbeit mit unseren deutschen Partnern!",
  // intro_video_description:
    // "Sehen Sie sich unser Einführungsvideo an und erfahren Sie mehr über unsere Sprachkurse und Angebote.",
  // intro_video_title: "Deutsch lernen: Die wichtigsten Sätze für Anfänger",

  // Photo Gallery Items
  // photo_alt_1: "Deutschkurs in Taschkent",
  // photo_caption_1: "Deutschkurs in Taschkent",
  // photo_alt_2: "Sprachzentrum in Samarkand",
  // photo_caption_2: "Unser neues Sprachzentrum in Samarkand",
  // photo_alt_3: "Studenten beim Lernen",
  // photo_caption_3: "Unsere Studenten beim Lernen",
  // photo_alt_4: "Kulturveranstaltung",
  // photo_caption_4: "Deutsche Kulturveranstaltung",
  // photo_alt_5: "Abschlussfeier",
  // photo_caption_5: "Abschlussfeier des B2-Kurses",
  // photo_alt_6: "Deutschlehrer",
  // photo_caption_6: "Unser Team von qualifizierten Deutschlehrern",

  // Language detection
  language_detected:
    "Wir haben die Seite automatisch in Ihrer Browsersprache angezeigt. Sie können die Sprache jederzeit ändern.",
  dismiss: "Schließen",

  // New translations for language-specific content
  popular_courses: "Beliebte Kurse",
  special_offers: "Sonderangebote",
  testimonial: "Erfahrungsbericht",

  // Social Media Feed
  all_platforms: "Alle Plattformen",
  view_more_posts: "Mehr Beiträge anzeigen",

  // Reviews
  found_helpful: "fanden dies hilfreich",
  helpful: "Hilfreich",
  show_more_reviews: "Mehr Bewertungen anzeigen",
  show_less: "Weniger anzeigen",
  leave_review: "Bewertung abgeben",
  write_review: "Bewertung schreiben",

  // Course descriptions
  course_desc_a1_beginner: "A1 – Grundlegende Sprachkenntnisse: Grundkenntnisse, einfache Sätze, alltägliche Ausdrücke",
  course_desc_a2_basic: "A2 – Schwellenwert der Sprachkenntnisse: Erweiterte Kenntnisse, direkte Kommunikation in alltäglichen Situationen",
  course_desc_b1_intermediate: "B1 – Fortgeschrittene Sprachkenntnisse: Flüssige Kommunikation, direkte Kommunikation in wichtigen Situationen (Ausbildung, Freizeit)",
  course_desc_b2_advanced: "B2 – Selbstständige Sprachkenntnisse: Flüssiges Sprechen, komplexe Texte, fachliche Diskussionen",
  course_desc_a1_a2_beginner_plus: "Für Anfänger und leicht Fortgeschrittene",
  telc_a2_level: "Für telc A2 Level",
  telc_a2_b1_level: "Für telc A2/B1 Level",
  telc_b1_level: "Für telc B1 Level",
  telc_b2_level: "Für telc B2 Level",
  telc_prep_title: "Vorbereitung auf die telc Prüfung",
  telc_prep_duration: "Kursdauer — 2 Monate",
  telc_prep_lesson_time: "Jede Unterrichtsstunde — 2 Std. 15 Min.",
  telc_prep_small_groups: "Kleine Gruppen — maximal 10 Teilnehmer",
  telc_prep_practice: "Praxisübungen im telc Prüfungsformat",
  telc_prep_materials: "Alle Lernmaterialien im Kurspreis enthalten",
  telc_duration_2_months: "Dauer 2 Monate",
  telc_two_hours: "2 Stunden",
  telc_max_8_students: "Maximal 8 Teilnehmer in einer Gruppe",

  // Group size indicators
  max_12_participants: " (max. 12 Teilnehmer)",
  max_10_participants: " (max. 10 Teilnehmer)",
  max_8_participants: " (max. 8 Teilnehmer)",

  // Language names
  language_german: "Deutsch",
  language_uzbek: "O'zbekcha",
  language_english: "English",
  language_russian: "Русский",

  // Alt texts and accessibility
  logo_alt: "Profi Deutsch Logo",
  switch_language: "Sprache wechseln",
  menu_close: "Menü schließen",
  menu_open: "Menü öffnen",
  previous_partners: "Vorherige Partner",
  next_partners: "Nächste Partner",

  // Partners section
  our_partners: "Unsere Partner",
  // partners_description:
  //   "Wir arbeiten mit führenden deutschen Unternehmen und Institutionen zusammen, um unseren Studenten die besten Möglichkeiten zu bieten.",
  visit_website: "Website besuchen",
  partnership_interest: "Interessiert an einer Partnerschaft? Kontaktieren Sie uns für weitere Informationen.",

  // Partner descriptions
  partner_telc_desc: "Weltweites Sprachprüfungszentrum",
  partner_duwk_desc: "Büro der Deutsch-Usbekischen Wirtschaftskooperation",
  partner_ssp_desc: "Industrie- und Handelskammer der Republik Usbekistan",
  partner_tavi_desc: "Regionalverwaltung Taschkent",
  partner_xb_desc: "Aktiengesellschaft Volksbank",
  partner_yosh_desc: "Jugendbehörde",
  partner_edu_desc: "Ministerium für Hochschulbildung, Wissenschaft und Innovation",
  partner_uzedu_desc: "Ministerium für Vorschul- und Schulbildung",
  partner_lufthansa_desc: "Deutsche Fluggesellschaft",

  // Special offers
  early_bird_discount: "Frühbucherrabatt",
  early_bird_desc: "Melden Sie sich 4 Wochen im Voraus an und erhalten Sie 15% Rabatt",

  // Teacher
  teacher_trained_in_germany: "In Deutschland ausgebildeter Pädagoge",

  // Locations
  tashkent_address: "STADT TASCHKENT, BEZIRK UCHTEPA, BOG'OBOD MFY, 15 MAVZESI, 14-UY",
  samarkand_address: "Samarqand shahri, O'zbekiston ko'chasi, 63 uy, 140105\n(Yoshlar kreativ shaharchasi)",

  // Courses
  course_german_a1: "Deutsch (A1)",
  course_german_a2: "Deutsch (A2)",
  course_german_b1: "Deutsch (B1)",
  course_german_b2: "Deutsch (B2)",
}

export default de
